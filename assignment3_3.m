...
for t = 0 : deltaT : tTotal  
    ...
    for i=1:N
        deltaPx(i)=vx(i)*deltaT+(1/2)*ax(floor(yPos(i)*10^9)+1,floor(xPos(i)*10^9)+1)*deltaT^2;
        deltaPy(i)=vy(i)*deltaT+(1/2)*ay(floor(yPos(i)*10^9)+1,floor(xPos(i)*10^9)+1)*deltaT^2
    end
